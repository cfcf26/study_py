...
def get_smartstore(store_url):
    ...
    item_options = driver.find_element_by_tag_name('body').find_elements_by_class_name(
        'bd_3hLoi')  # 옵션 선택값 전체선택 태그 네임으로 가져올때는 html상의 앞에 지정된 스타일에서 태그네임을 먼저 가져와서 오류가 생김으로 반드시 body에서 가져와야 문제가 생기지 않는다
    ...
    for j in item_option_list_1:
        ...
        item_options[1].click()  # 옵션2 버튼 클릭
        itme_option_list_2.append(item_options[1].text)  # 옵션 2의 전체 택스트 리스트에 저장
        item_options[0].click()  # 옵션1 버튼 클릭
